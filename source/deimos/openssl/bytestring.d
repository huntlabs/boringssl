module deimos.openssl.bytestring;
...
// CBS_get_u8 sets |*out_ptr| to the next uint8_t from |cbs| and advances |cbs|. It
// returns one on success and zero on error.
int CBS_get_u8(CBS *cbs, uint8_t *out_ptr);

// CBS_get_u16 sets |*out_ptr| to the next, big-endian uint16_t from |cbs| and
// advances |cbs|. It returns one on success and zero on error.
int CBS_get_u16(CBS *cbs, uint16_t *out_ptr);

// CBS_get_u24 sets |*out_ptr| to the next, big-endian 24-bit value from |cbs| and
// advances |cbs|. It returns one on success and zero on error.
int CBS_get_u24(CBS *cbs, uint32_t *out_ptr);

// CBS_get_u32 sets |*out_ptr| to the next, big-endian uint32_t value from |cbs|
// and advances |cbs|. It returns one on success and zero on error.
int CBS_get_u32(CBS *cbs, uint32_t *out_ptr);

// CBS_get_last_u8 sets |*out_ptr| to the last uint8_t from |cbs| and shortens
// |cbs|. It returns one on success and zero on error.
int CBS_get_last_u8(CBS *cbs, uint8_t *out_ptr);

// CBS_get_bytes sets |*out_ptr| to the next |len| bytes from |cbs| and advances
// |cbs|. It returns one on success and zero on error.
...
// CBS_get_u8_length_prefixed sets |*out_ptr| to the contents of an 8-bit,
// length-prefixed value from |cbs| and advances |cbs| over it. It returns one
// on success and zero on error.
int CBS_get_u8_length_prefixed(CBS *cbs, CBS *out_ptr);

// CBS_get_u16_length_prefixed sets |*out_ptr| to the contents of a 16-bit,
// big-endian, length-prefixed value from |cbs| and advances |cbs| over it. It
// returns one on success and zero on error.
int CBS_get_u16_length_prefixed(CBS *cbs, CBS *out_ptr);

// CBS_get_u24_length_prefixed sets |*out_ptr| to the contents of a 24-bit,
// big-endian, length-prefixed value from |cbs| and advances |cbs| over it. It
// returns one on success and zero on error.
...
enum CBS_ASN1_BOOLEAN = 0x1;
enum CBS_ASN1_INTEGER = 0x2;
enum CBS_ASN1_BITSTRING = 0x3;
enum CBS_ASN1_OCTETSTRING = 0x4;
enum CBS_ASN1_NULL = 0x5;
enum CBS_ASN1_OBJECT = 0x6;
enum CBS_ASN1_ENUMERATED = 0xa;
enum CBS_ASN1_UTF8STRING = 0xc;
enum CBS_ASN1_SEQUENCE = (0x10 | CBS_ASN1_CONSTRUCTED);
enum CBS_ASN1_SET = (0x11 | CBS_ASN1_CONSTRUCTED);
enum CBS_ASN1_NUMERICSTRING = 0x12;
enum CBS_ASN1_PRINTABLESTRING = 0x13;
enum CBS_ASN1_T61STRING = 0x14;
enum CBS_ASN1_VIDEOTEXSTRING = 0x15;
enum CBS_ASN1_IA5STRING = 0x16;
enum CBS_ASN1_UTCTIME = 0x17;
enum CBS_ASN1_GENERALIZEDTIME = 0x18;
enum CBS_ASN1_GRAPHICSTRING = 0x19;
enum CBS_ASN1_VISIBLESTRING = 0x1a;
enum CBS_ASN1_GENERALSTRING = 0x1b;
enum CBS_ASN1_UNIVERSALSTRING = 0x1c;
enum CBS_ASN1_BMPSTRING = 0x1e;
...
// tag.
enum CBS_ASN1_CONSTRUCTED = (0x20 << CBS_ASN1_TAG_SHIFT);
...
enum CBS_ASN1_UNIVERSAL = (0 << CBS_ASN1_TAG_SHIFT);
enum CBS_ASN1_APPLICATION = (0x40 << CBS_ASN1_TAG_SHIFT);
enum CBS_ASN1_CONTEXT_SPECIFIC = (0x80 << CBS_ASN1_TAG_SHIFT);
enum CBS_ASN1_PRIVATE = (0xc0 << CBS_ASN1_TAG_SHIFT);

// CBS_ASN1_CLASS_MASK may be ANDed with a tag to query its class. This will
// give one of the four values above.
enum CBS_ASN1_CLASS_MASK = (0xc0 << CBS_ASN1_TAG_SHIFT);
...
// CBS_get_asn1 sets |*out_ptr| to the contents of DER-encoded, ASN.1 element (not
...
// CBS_get_any_asn1 sets |*out_ptr| to contain the next ASN.1 element from |*cbs|
...
// CBS_get_any_asn1_element sets |*out_ptr| to contain the next ASN.1 element from
// |*cbs| (including header bytes) and advances |*cbs|. It sets |*out_tag| to
// the tag number and |*out_header_len| to the length of the ASN.1 header. Each
// of |out|, |out_tag|, and |out_header_len| may be NULL to ignore the value.
int CBS_get_any_asn1_element(CBS *cbs, CBS *out_ptr,
                                            uint *out_tag,
...
int CBS_get_any_ber_asn1_element(CBS *cbs, CBS *out_ptr,
                                                uint *out_tag,
                                                size_t *out_header_len);

// CBS_get_asn1_uint64 gets an ASN.1 INTEGER from |cbs| using |CBS_get_asn1|
// and sets |*out_ptr| to its value. It returns one on success and zero on error,
// where error includes the integer being negative, or too large to represent
// in 64 bits.
int CBS_get_asn1_uint64(CBS *cbs, uint64_t *out_ptr);

// CBS_get_asn1_bool gets an ASN.1 BOOLEAN from |cbs| and sets |*out_ptr| to zero
...
// tagged with |tag| and sets |*out_ptr| to its contents, or ignores it if |out| is
...
int CBS_get_optional_asn1(CBS *cbs, CBS *out_ptr, int *out_present,
                                         uint tag);

// CBS_get_optional_asn1_octet_string gets an optional
// explicitly-tagged OCTET STRING from |cbs|. If present, it sets
// |*out_ptr| to the string and |*out_present| to one. Otherwise, it sets
// |*out_ptr| to empty and |*out_present| to zero. |out_present| may be
// NULL. It returns one on success, whether or not the element was
// present, and zero on decode failure.
int CBS_get_optional_asn1_octet_string(CBS *cbs, CBS *out_ptr,
                                                      int *out_present,
                                                      uint tag);

// CBS_get_optional_asn1_uint64 gets an optional explicitly-tagged
// INTEGER from |cbs|. If present, it sets |*out_ptr| to the
// value. Otherwise, it sets |*out_ptr| to |default_value|. It returns one
// on success, whether or not the element was present, and zero on
// decode failure.
int CBS_get_optional_asn1_uint64(CBS *cbs, uint64_t *out_ptr,
                                                uint tag,
                                                uint64_t default_value);

// CBS_get_optional_asn1_bool gets an optional, explicitly-tagged BOOLEAN from
// |cbs|. If present, it sets |*out_ptr| to either zero or one, based on the
// boolean. Otherwise, it sets |*out_ptr| to |default_value|. It returns one on
// success, whether or not the element was present, and zero on decode
// failure.
int CBS_get_optional_asn1_bool(CBS *cbs, int *out_ptr, uint tag,
                                              int default_value);
...
int CBB_add_asn1_oid_from_text(CBB *cbb, const char *text,
                                              size_t len);
